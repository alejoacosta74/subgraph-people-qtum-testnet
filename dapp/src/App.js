...
const GRAVATARS_QUERY = gql`
  query gravatars($where: Person_filter!, $orderBy: Person_orderBy!) {
    persons(first: 100, where: $where, orderBy: $orderBy, orderDirection: asc) {
      id
      owner
      displayName
      imageUrl
                ...
                >
                  {({ data, error, loading }) => {
                    return loading ? (
                      ...
                      <Gravatars gravatars={data.persons} />
                    )
                  }}
                </Query>
              </Grid>
